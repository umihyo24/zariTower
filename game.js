const CONFIG = {
  ...
  haginoinoshishi: {
    ...
    boss: {
      ...
      exhaustedDuration: 2.0, finalExhaustedDuration: 8.0, walkSpeed: 70, chargeSpeedEarly: 268, chargeSpeedRage: 292,
      chargeTurnRateEarly: 0.78, chargeTurnRateRage: 1.02, chargeAimLockAtPrepare: 0.92, chargeMaxCorrectionDegrees: 20, chargeCorrectionFalloff: 0.74, ruinedGroundChargeTurnRateMultiplier: 0.5,
    ...
    ruinedGround: {
      trailSpawnDistance: 26, chargeTrailRadius: 38, walkTrailRadius: 22, trailDeepenRadius: 46, trailDeepenAmount: 0.22, maxTrailPatches: 96,
      trailLifetime: 16, trailRecoveryPerSecond: 0.06, freshSlowMultiplier: 0.9, deepSlowMultiplier: 0.62, minimumRuinedGroundSpeedMultiplier: 0.52,
...
function startHaginoinoshishiEncounter() {
  ...
  gameState.duel = { active: true, isComplete: false, timer: 0, bossType: 'haginoinoshishi', ruinedGround: [], duelParticles: [], cameraShakeTimer: 0, cameraShakeStrength: 0,
    boss: { id:'duel_haginoinoshishi', x:Number(a.bossStartX)||760, y:Number(a.bossStartY)||260, radius:Number(bcfg.radius)||76, hp:Number(bcfg.maxHpForBarOrDisplay)||100, maxHp:Number(bcfg.maxHpForBarOrDisplay)||100,
      ...
      lastTrailX:Number(a.bossStartX)||760, lastTrailY:Number(a.bossStartY)||260, trailCarry:0, chargeCorrectionBudget:0, damageTimer:0, fatigue:0, fatigueBonusWindow:0, chargeMissTimer:0, slideTime:0, resistantFlashTimer:0, weakPointFlashTimer:0 }
  };
}
...
function startMatsuruEncounter() {
  ...
  gameState.duel = {
    ...
    boss: {
      ...
      openingDone: false, matsukazeTargetX: Number(c.matsuruX) || 770, matsukazeTargetY: Number(c.matsuruY) || 270,
...
function updateHaginoinoshishiBossBattle(dt) { const duel=gameState?.duel; const p=gameState?.player; const cfg=CONFIG.haginoinoshishi||{}; const a=cfg.arena||{}; const c=cfg.boss||{}; const rg=cfg.ruinedGround||{}; const b=duel?.boss; if(!duel||!p||!b)return; const inv=Boolean(gameState?.debug?.enabled)&&Boolean(gameState?.debug?.playerInvincible); updatePlayerMovement(dt); updatePlayerAttack(dt); updateProjectiles(dt); updateParticles(dt); duel.timer=(duel.timer||0)+dt; b.stateTimer=Math.max(0,(b.stateTimer||0)-dt); b.damageTimer=Math.max(0,(b.damageTimer||0)-dt); b.resistantFlashTimer=Math.max(0,(b.resistantFlashTimer||0)-dt); b.weakPointFlashTimer=Math.max(0,(b.weakPointFlashTimer||0)-dt); const px=Number(p.x)||0,py=Number(p.y)||0; const dx=px-(b.x||0),dy=py-(b.y||0); const dist=Math.hypot(dx,dy)||1; const fatigueRatio=clamp((Number(b.fatigue)||0)/Math.max(1,Number(c.fatigueRequiredForFinal)||100),0,1); const rage=fatigueRatio>=(Number(c.rageThreshold)||0.6);
 ...
 const ruin=sample(p.x,p.y); const fresh=Number(rg.freshSlowMultiplier)||0.9; const deep=Number(rg.deepSlowMultiplier)||0.62; const minSlow=Number(rg.minimumRuinedGroundSpeedMultiplier)||0.52; p.contactSlowMultiplier=clamp((Number(rg.playerCleanGroundSpeedMultiplier)||1)*(fresh+(deep-fresh)*ruin),minSlow,1); p.turnPenaltyMultiplier=clamp(1-ruin*(1-(Number(rg.turnPenaltyMultiplier)||0.72)),Number(rg.turnPenaltyMultiplier)||0.72,1);
 if(b.state==='idle'){b.facing=Math.atan2(dy,dx); b.facingX=Math.cos(b.facing); b.x += Math.cos(b.facing)*(Number(c.walkSpeed)||70)*dt; b.y += Math.sin(b.facing)*(Number(c.walkSpeed)||70)*dt; addTrail(b.x,b.y,Number(rg.walkTrailRadius)||22,0.25); if(b.stateTimer<=0){b.state='prepareCharge'; b.stateTimer=(Number(c.prepareDuration)||1)*(rage?(Number(c.ragePrepareMultiplier)||0.8):1);} }
 else if(b.state==='prepareCharge'){const lock=clamp(Number(c.chargeAimLockAtPrepare)||0.92,0,1); b.lockedTargetX=(b.lockedTargetX||px)*(1-lock)+px*lock; b.lockedTargetY=(b.lockedTargetY||py)*(1-lock)+py*lock; const ldx=b.lockedTargetX-(b.x||0), ldy=b.lockedTargetY-(b.y||0); const ldist=Math.hypot(ldx,ldy)||1; b.chargeDirX=ldx/ldist; b.chargeDirY=ldy/ldist; b.facing=Math.atan2(b.chargeDirY,b.chargeDirX); b.facingX=b.chargeDirX; addTrail(b.x+rand(-8,8),b.y+rand(-8,8),Number(rg.walkTrailRadius)||22,0.3); if(b.stateTimer<=0){b.state='charging'; b.stateTimer=Number(c.chargeDuration)||2.1; b.chargeMissTimer=0; b.slideTime=0; b.lastTrailX=b.x; b.lastTrailY=b.y; b.trailCarry=0; b.chargeCorrectionBudget=(Number(c.chargeMaxCorrectionDegrees)||20)*(Math.PI/180);} }
 else if(b.state==='charging'){const localR=sample(b.x,b.y); const baseTurn=rage?(Number(c.chargeTurnRateRage)||1.02):(Number(c.chargeTurnRateEarly)||0.78); const turn=Math.max(0,baseTurn*(1-localR*(1-(Number(c.ruinedGroundChargeTurnRateMultiplier)||0.5)))); const targetA=Math.atan2(dy,dx), currA=Math.atan2(b.chargeDirY||0,b.chargeDirX||1); let da=((targetA-currA+Math.PI*3)%(Math.PI*2))-Math.PI; const budget=Math.max(0,Number(b.chargeCorrectionBudget)||0); da=clamp(da,-budget,budget); const step=clamp(turn*dt*(1-clamp((Number(c.chargeCorrectionFalloff)||0.74)*fatigueRatio,0,0.85)),0,Math.PI); const applied=clamp(da,-step,step); const na=currA+applied; b.chargeDirX=Math.cos(na); b.chargeDirY=Math.sin(na); b.chargeCorrectionBudget=Math.max(0,budget-Math.abs(applied)); b.facing=Math.atan2(b.chargeDirY,b.chargeDirX); b.facingX=b.chargeDirX; const baseSpeed=rage?(Number(c.chargeSpeedRage)||292):(Number(c.chargeSpeedEarly)||268); const speed=clamp(baseSpeed*(rage?(Number(c.rageSpeedMultiplier)||1.1):1)*(1+localR*((Number(rg.boarSlideMultiplierOnRuinedGround)||1.18)-1)),120,450); b.x += b.chargeDirX*speed*dt; b.y += b.chargeDirY*speed*dt; const seg=Math.hypot((b.x||0)-(b.lastTrailX||b.x),(b.y||0)-(b.lastTrailY||b.y)); b.trailCarry=(Number(b.trailCarry)||0)+seg; const trailDist=Math.max(6,Number(rg.trailSpawnDistance)||26); let spawns=0; while(b.trailCarry>=trailDist&&spawns<3){b.trailCarry-=trailDist; const nx=(b.lastTrailX||b.x)+b.chargeDirX*trailDist, ny=(b.lastTrailY||b.y)+b.chargeDirY*trailDist; addTrail(nx,ny,Number(rg.chargeTrailRadius)||38,1); b.lastTrailX=nx; b.lastTrailY=ny; spawns++;} b.chargeMissTimer += dt; if(localR>0.5) b.slideTime += dt; if(b.stateTimer<=0||b.x<=b.radius||b.x>=(Number(a.arenaWidth)||960)-b.radius||b.y<=b.radius||b.y>=(Number(a.arenaHeight)||540)-b.radius){b.fatigue=clamp((b.fatigue||0)+(Number(c.fatigueFromLongCharge)||10)+(b.chargeMissTimer>1.25?(Number(c.fatigueFromChargeMiss)||18):0)+(b.slideTime>0.45?(Number(c.fatigueFromSlidingOnRuinedGround)||14):0),0,Number(c.fatigueRequiredForFinal)||100); b.state=(b.fatigue>=(Number(c.fatigueRequiredForFinal)||100))?'finalExhausted':'exhausted'; b.stateTimer=(b.state==='finalExhausted'?(Number(c.finalExhaustedDuration)||8):(Number(c.exhaustedDuration)||2))*(rage?(Number(c.rageCooldownMultiplier)||0.86):1);} }
...
function updateMatsuruBossBattle(dt) {
  ...
  if (b.state === 'openingMatsuba') {
    ...
    b.stateTimer = debugFastMode ? (Number(CONFIG.debug?.matsuruPrepareDurationFast) || c.matsuruPrepareDuration || 0.8) : (c.matsuruPrepareDuration || 0.8);
    b.attackText = '松風の構え…';
  ...
  else if (b.state === 'prepareMatsukaze' && b.stateTimer <= 0) { b.state = 'matsukaze'; b.stateTimer = c.matsuruWindDuration || 1.5; b.windX = b.x; const lead = Number(c.matsuruMatsukazeTargetLockAhead) || 0; b.matsukazeTargetX = clamp((p.x || 0) + (p.vx || 0) * lead, p.radius, CONFIG.canvas.width - p.radius); b.matsukazeTargetY = clamp((p.y || 0) + (p.vy || 0) * lead, p.radius, CONFIG.canvas.height - p.radius); b.windVx = b.matsukazeTargetX < b.x ? -1 : 1; b.multiHitTimer = 0; b.matsukazePulseDelayTimer = clamp(Number(c.matsukazePulseSynergyDelay) || 0, 0, Number(c.matsubaPulseInterval) || 2.1); }
  ...
  if (b.state === 'matsukaze' || b.state === 'matsukazeReturn') {
    b.windX += b.windVx * (CONFIG.canvas.width * dt * 0.7); b.multiHitTimer = Math.max(0, (b.multiHitTimer || 0) - dt); const windHit = Math.abs(p.x - b.windX) <= (Number(c.matsuruWindWidth) || 160) && Math.abs(p.y - b.matsukazeTargetY) <= (Number(c.matsuruWindHeight) || 120);
  ...
  let movingCount = 0;
  const fireFromTurret = (turret, spreadAngleOverride = null, jitterStrength = 0) => {
    const spread = Number.isFinite(spreadAngleOverride) ? spreadAngleOverride : (Number(c.matsuruTurretBaseSpreadAngle) || 0.28); const baseAngle = Number(turret?.baseAngle) || Math.PI; const maxCorr = Number(c.matsuruTurretMaxCorrectionAngle) || 0.38;
    const toPlayer = Math.atan2((p.y || 0) - (turret.y || 0), (p.x || 0) - (turret.x || 0)); let delta = toPlayer - baseAngle;
    while (delta > Math.PI) delta -= Math.PI * 2; while (delta < -Math.PI) delta += Math.PI * 2;
    const aimNoise = Number(c.matsuruTurretAimRandomness) || 0;
    const corrected = baseAngle + clamp(delta, -maxCorr, maxCorr) + rand(-jitterStrength, jitterStrength) + rand(-aimNoise, aimNoise);
    const wallAvoid = clamp(Number(c.matsuruTurretWallAvoidanceStrength) || 0, 0, 0.8);
    const centerAngle = Math.atan2((CONFIG.canvas.height * 0.5) - turret.y, (CONFIG.canvas.width * 0.5) - turret.x);
    const finalBase = corrected * (1 - wallAvoid) + centerAngle * wallAvoid;
    for (let i = 0; i < 2; i += 1) {
      const angle = finalBase + (i === 0 ? -spread : spread);
      b.needles.push({ x: turret.x, y: turret.y, vx: Math.cos(angle) * (Number(c.matsuruLeafSpeed) || 122) * (Number(c.matsuruTurretLeafSpawnSpeedScale) || 0.9), vy: Math.sin(angle) * (Number(c.matsuruLeafSpeed) || 122) * (Number(c.matsuruTurretLeafSpawnSpeedScale) || 0.9), radius: Number(c.MATSUBA_PROJECTILE_RADIUS) || 7, life: Number(c.matsuruLeafLifetime) || 6.2, damage: (Number(c.matsuruNeedleDamage) || 10) * (Number(c.matsuruTurretLeafDamageScale) || 0.8), angle, spin: rand(-2.4, 2.4), sway: rand(0.7, 1.3), type: 'moving', source: 'turret' });
    }
  };
  b.needles.forEach(n => {
  ...
  if (turrets.length > maxActiveTurrets) turrets.sort((a, z) => (z.age || 0) - (a.age || 0)).slice(maxActiveTurrets).forEach(n => { n.type = 'dormant'; n.activationTimer = 2.2; });
  ...
  const spread = ((Number(c.matsubaPulseSpreadDegrees) || 16) * Math.PI) / 180;
  const fireJitter = clamp(Number(c.matsubaPulseFireJitter) || 0.1, 0, 0.5);
  const maxTurretsPerWave = clamp(Math.floor(Number(c.matsubaPulseMaxTurretsPerWave) || 10), 1, maxActiveTurrets);
  if (b.matsubaPulseTimer <= 0) {
    const waveTurrets = turrets.slice().sort((a, z) => (a.age || 0) - (z.age || 0)).slice(0, maxTurretsPerWave);
    let firedProjectiles = 0;
    waveTurrets.forEach((turret) => { fireFromTurret(turret, spread, fireJitter); firedProjectiles += 2; });
...
function renderHaginoinoshishiBossBattle() { const duel=gameState?.duel; const b=duel?.boss; const cfg=CONFIG.haginoinoshishi||{}; const rg=cfg.ruinedGround||{}; const v=cfg.visuals||{}; if(!b)return; for(const t of (duel?.ruinedGround||[])){const a=clamp((t.ruinLevel||0)/(Number(rg.maxRuinLevel)||1),0,1); ctx.fillStyle = a>0.55 ? (v.trailColorDeep||'rgba(66,45,32,0.45)') : (v.trailColorFresh||'rgba(111,82,61,0.22)'); ctx.beginPath(); ctx.arc(t.x||0,t.y||0,t.radius||20,0,Math.PI*2); ctx.fill(); }
 ...
 const dbg=gameState?.debug||{}; const cfgBoss=CONFIG.haginoinoshishi?.boss||{}; const localR=(duel?.ruinedGround||[]).reduce((m,t)=>{if(!t)return m; const dd=Math.hypot((b.x||0)-(t.x||0),(b.y||0)-(t.y||0)); return dd<=(t.radius||0)?Math.max(m,Number(t.ruinLevel)||0):m;},0); const fatigueRatio=clamp((Number(b.fatigue)||0)/Math.max(1,Number(cfgBoss.fatigueRequiredForFinal)||100),0,1); const rage=fatigueRatio>=(Number(cfgBoss.rageThreshold)||0.6); const turn=(rage?(Number(cfgBoss.chargeTurnRateRage)||1.02):(Number(cfgBoss.chargeTurnRateEarly)||0.78))*(1-localR*(1-(Number(cfgBoss.ruinedGroundChargeTurnRateMultiplier)||0.5)));
 ctx.fillStyle='rgba(10,16,24,0.65)'; ctx.fillRect(560,14,390,88); ctx.fillStyle='#f4f7f9'; ctx.fillText(`HAGINOINOSHISHI ${b.state}`,574,34); ctx.fillText(`Fatigue ${Math.floor(b.fatigue||0)} / ${Math.floor(cfgBoss.fatigueRequiredForFinal||100)} | Turn ${turn.toFixed(2)} | Ruin ${(duel?.ruinedGround||[]).length}`,574,54); ctx.fillText(`PlayerSpeed x${(Number(gameState?.player?.contactSlowMultiplier)||1).toFixed(2)} | Invincible ${Boolean(dbg.enabled&&dbg.playerInvincible)?'ON':'OFF'}`,574,74); }
...
function drawHud() {
  ...
  if (showDebug && gameState?.duel?.bossType === 'matsuru') {
    ...
    ctx.fillText(`Pulse Timer: ${(Number(b?.matsubaPulseTimer) || 0).toFixed(2)}`, 300, 510);
    ctx.fillText(`Turrets: ${activeTurrets}`, 300, 526);
    ctx.fillText(`Stuck Leaves: ${stuckLeaves}`, 300, 542);
    ctx.fillText(`Projectiles: ${leaves.filter(n => n && n.type === 'moving').length}`, 300, 558);
    ctx.fillText(`Speed Mult: ${speedMul.toFixed(2)}x`, 300, 574);
  }
}
